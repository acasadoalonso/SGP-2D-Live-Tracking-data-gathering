...
--
-- Host: localhost:3306
-- Generation Time: Jun 22, 2020 at 05:16 PM
...
CREATE TABLE `GLIDERS_POSITIONS` (
  ...
  `gps` char(6) DEFAULT 'NO',
  `lastFixTx` datetime DEFAULT CURRENT_TIMESTAMP,
...
--
-- Stand-in structure for view `GUTMAAIR`
-- (See below for the actual view)
--
DROP VIEW IF EXISTS `GUTMAAIR`;
CREATE TABLE `GUTMAAIR` (
`idflarm` char(9)
,`latitude` float
,`longitude` float
,`altitude` int(11)
...
CREATE TABLE `MASTER_COLORS` (
  ...
  `idFlarm` char(9) NOT NULL DEFAULT ''
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `OGNTRKSTATUS` (
  ...
  `source` varchar(4) DEFAULT 'OGN' COMMENT 'Source of the data'
...
CREATE TABLE `OPS` (
  ...
  `takeOffAirport` varchar(10) NOT NULL DEFAULT '',
  `takeOffDate` char(6) NOT NULL DEFAULT '',
  `takeOffTime` char(6) NOT NULL DEFAULT '',
  `landingAirport` varchar(10) NOT NULL DEFAULT '',
  `landingDate` char(6) DEFAULT '',
  `landingTime` char(6) DEFAULT ''
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
--
-- Structure for view `GUTMAAIR`
--
DROP TABLE IF EXISTS `GUTMAAIR`;

CREATE ALGORITHM=UNDEFINED DEFINER=`ogn`@`%` SQL SECURITY DEFINER VIEW `GUTMAAIR`  AS  select `OGNDATA`.`idflarm` AS `idflarm`,`OGNDATA`.`latitude` AS `latitude`,`OGNDATA`.`longitude` AS `longitude`,`OGNDATA`.`altitude` AS `altitude`,`OGNDATA`.`course` AS `course`,`OGNDATA`.`speed` AS `speed`,`OGNDATA`.`roclimb` AS `roclimb`,`OGNDATA`.`uniqueid` AS `uniqueid`,`OGNDATA`.`date` AS `date`,`OGNDATA`.`time` AS `time`,(select `GLIDERS`.`registration` from `GLIDERS` where (`GLIDERS`.`idglider` = `OGNDATA`.`idflarm`)) AS `reg` from `OGNDATA` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`ogn`@`%` SQL SECURITY DEFINER VIEW `TRKDEV`  AS  select `TRKDEVICES`.`id` AS `id`,`TRKDEVICES`.`owner` AS `owner`,`TRKDEVICES`.`spotid` AS `spotid`,`TRKDEVICES`.`spotpasswd` AS `spotpasswd`,`TRKDEVICES`.`compid` AS `compid`,`TRKDEVICES`.`model` AS `model`,`TRKDEVICES`.`registration` AS `registration`,`TRKDEVICES`.`active` AS `active`,`TRKDEVICES`.`devicetype` AS `devicetype`,`TRKDEVICES`.`flarmid` AS `flarmid`,(select `GLIDERS`.`registration` from `GLIDERS` where (`GLIDERS`.`idglider` = substr(`TRKDEVICES`.`flarmid`,4,6))) AS `GLID_REG`,(select `GLIDERS`.`registration` from `GLIDERS` where (`GLIDERS`.`idglider` = substr(`TRKDEVICES`.`id`,4,6))) AS `OGN_REG`,`TRKDEVICES`.`registration` AS `regis` from `TRKDEVICES` where (`TRKDEVICES`.`active` = 1) ;
...
--
ALTER TABLE `GLIDERS`
  ADD PRIMARY KEY (`idglider`) USING BTREE;
...
ALTER TABLE `OGNDATAARCHIVE`
  ADD KEY `datetime` (`idflarm`,`date`,`time`,`station`) USING BTREE,
  ADD KEY `OGNDIDX` (`idflarm`,`date`) USING BTREE;
...
--
ALTER TABLE `OPS`
  MODIFY `idOP` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT for table `WAYPOINTS`
--
ALTER TABLE `WAYPOINTS`
  MODIFY `idWaypoint` int(11) NOT NULL AUTO_INCREMENT;
