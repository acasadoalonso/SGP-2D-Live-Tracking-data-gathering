...
def getddbdata(prt=False):                  		# get the data from the API server
    ...
    try:
       ...
       j_obj=''
    if len(j_obj) == 0:			    # if not ognddb 
       print("Empty ... Using the .json file")
       r=open("ognddbdata.json")	    # open the file generated by wget
       js=r.read()
       j_obj = json.loads(js)               # convert to JSON
       r.close()			    # close the file now
